...
?>
    ...
    foreach ($footer_templates as $post) {
        ...
        add_action('wp_footer', function () use ($footer_template_id, $ct_template_condition, $footer_specific_page_slug, &$particular_templates) {
            if (class_exists('Elementor\Plugin') && !is_admin()) {
    ...
    function display_template_post_notice()
    {
        if (get_transient('ct_header_footer_template_registered_condition')) {
            ...
            <div class="notice notice-error is-dismissible">
                <p><?php echo __('Tempate has been already registered for this location/page!', 'cozy-addons'); ?> </p>
            </div>
            <?php
        }
    }
    ...
    if (!class_exists('Cozy_Header_Footer_Notice_Menu')) :
